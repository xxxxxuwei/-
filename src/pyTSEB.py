...
要将TSEB配置为处理表列数据的时间序列，请键入ipython终端或jupyter笔记本
.. code-block:: ipython
...
class PyTSEB():
    ...
    def __init__(self):
        '''Initialize input variables  with default  values用默认值初始化输入变量'''
        ...
        self.emisVeg=0.98
        self.emisGrd=0.95
        # Surface Properties表面性质
        self.max_PT=1.26
        self.x_LAD=1.0
        self.leaf_width=0.1
        self.z0soil=0.01
        self.LANDCOVER=11
        # Soil Heat Flux calculation土壤热通量计算
        self.CalcG=1
        self.Gconstant=0
        self.Gratio=0.35
        self.GAmp=0.35
        self.Gphase=3
        self.Gshape=24
        # Default Vegetation variables默认的植被变量
        self.f_c=1.0
        self.f_g=1.0
        self.wc=1.0
        # Output variables saved in images保存在图像中的输出变量
        self.fields=('H1','LE1','R_n1','G1')
        # Ancillary output variables辅助输出变量
        self.anc_fields=('H_C1','LE_C1','LE_partition','T_C1', 'T_S1','R_ns1','R_nl1', 'u_friction', 'L')
        # File Configuration variables文件配置变量
    ...
    def PointTimeSeriesWidget(self):
        ...
        为点时间序列数据集创建用于运行TSEB的jupyter笔记本GUI'''
