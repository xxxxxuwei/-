...
def  TSEB_PT(Tr_K,vza,Ta_K,u,ea,p,Sdn_dir, Sdn_dif, fvis,fnir,sza,Lsky,
    ...
    利用单个复合辐射温度观测和串联电阻计算Priestley Taylor TSEB通量
    Parameters
    ----------
    Tr_K : float
        Radiometric composite temperature (Kelvin).放射复合温度
    vza : float
        View Zenith Angle (degrees).观测天顶角
    Ta_K : float 
        Air temperature (Kelvin).空气温度
    u : float 
        Wind speed above the canopy (m s-1).冠层上方的风速
    ea : float
        Water vapour pressure above the canopy (mb).树冠上方的水蒸气压力
    p : float
        Atmospheric pressure (mb), use 1013 mb by default.大气压力
    Sdn_dir : float
        Beam solar irradiance (W m-2).直射辐照度
    Sdn_dif : float
        Difuse solar irradiance (W m-2).散射辐照度
    fvis : float
        Fraction of shortwave radiation corresponding to the PAR region (400-700nm).与光合有效辐射区域相对应的短波辐射的比例
    fnir : float
        Fraction of shortwave radiation corresponding to the NIR region (700-2500nm).与近红外区域相对应的短波辐射的比例
    sza : float
        Solar Zenith Angle (degrees).太阳天顶角
    Lsky : float
        Downwelling longwave radiation (W m-2).下行长波辐射
    LAI : float
        Effective Leaf Area Index (m2 m-2).有效叶面积指数
    hc : float
        Canopy height (m).林冠高度
    emisVeg : float
        Leaf emissivity.叶片发射率
    emisGrd : flaot
        Soil emissivity.土壤发射率
    spectraVeg : dict('rho_leaf_vis', 'tau_leaf_vis', 'rho_leaf_nir','tau_leaf_nir')
        Leaf spectrum dictionary.  叶片光谱字典      

            rho_leaf_vis : float
                leaf bihemispherical reflectance in the visible (400-700 nm).可见光波段的叶片双半球反射率
            tau_leaf_vis : float
                leaf bihemispherical transmittance in the visible (400-700nm).叶片在可见光波段的双半球透射率
            rho_leaf_nir : float
                leaf bihemispherical reflectance in the optical infrared (700-2500nm),光学红外中的叶片双半球反射
            tau_leaf_nir : float
                leaf bihemispherical transmittance in the optical  infrared (700-2500nm).光学红外中叶片的双半球透射率
    spectraGrd : dict('rho rsoilv', 'rsoiln')
        Soil spectrum dictonary.土壤光谱指示
        
            rsoilv : float
                soil bihemispherical reflectance in the visible (400-700 nm).可见光下土壤双半球反射
            rsoiln : float
                soil bihemispherical reflectance in the optical infrared (700-2500nm).土壤在光红外波段的双半球反射率
    z_0M : float
        Aerodynamic surface roughness length for momentum transfer 动量传递的气动表面粗糙度长度(m).
    d_0 : float
        Zero-plane displacement height (m).零平面位移高度
    zu : float
        Height of measurement of windspeed (m).风速测量高度
    zt : float
        Height of measurement of air temperature (m).气温测量高度
    leaf_width : float, optional
        average/effective leaf width (m).平均/有效叶宽
    z0_soil : float, optional
        bare soil aerodynamic roughness length (m).裸露土壤气动粗糙度长度
    alpha_PT : float, optional
        Priestley Taylor coeffient for canopy potential transpiration, 林冠潜在蒸腾的Priestley-Taylor系数
        use 1.26 by default.
    x_LAD : float, optional
        Campbell 1990 leaf inclination distribution function chi parameter.Campbell 1990叶倾角分布函数chi参数
    f_c : float, optional
        Fractional cover.部分覆盖
    f_g : float, optional
        Fraction of vegetation that is green.绿色植被的比例
    wc : float, optional
        Canopy width to height ratio.冠层宽高比
    CalcG : tuple(int,list), optional
        Method to calculate soil heat flux,parameters.计算土壤热通量的方法、参数
    ...
    -------
    flag : int
        Quality flag, see Appendix for description.质量标志
    Ts : float
        Soil temperature  (Kelvin).土壤温度
    Tc : float
        Canopy temperature  (Kelvin).冠层温度
    T_AC : float
        Air temperature at the canopy interface (Kelvin).冠层界面的气温
    S_nS : float
        Soil net shortwave radiation (W m-2)土壤净短波辐射
    S_nC : float
        Canopy net shortwave radiation (W m-2)冠层净短波辐射
    L_nS : float
        Soil net longwave radiation (W m-2)土壤净长波辐射
    L_nC : float
        Canopy net longwave radiation (W m-2)冠层净长波辐射
    LE_C : float
        Canopy latent heat flux (W m-2).冠层潜热通量
    H_C : float
        Canopy sensible heat flux (W m-2).冠层感热通量
    LE_S : float
        Soil latent heat flux (W m-2).土壤潜热通量
    H_S : float
        Soil sensible heat flux (W m-2).土壤感热通量
    G : float
        Soil heat flux (W m-2).土壤热通量
    R_s : float
        Soil aerodynamic resistance to heat transport (s m-1).土壤对热传输的空气动力阻力
    R_x : float
        Bulk canopy aerodynamic resistance to heat transport (s m-1).对热传输的体积冠层空气动力阻力
    R_a : float
        Aerodynamic resistance to heat transport (s m-1).热传输的空气动力阻力
    u_friction : float
        Friction velocity (m s-1).摩擦速度
    L : float
        Monin-Obuhkov length (m).Monin-Obuhkov长度
    n_iterations : int
        number of iterations until convergence of L.迭代次数，直到L收敛
